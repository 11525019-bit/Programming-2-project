# Start Define
# directory files
# the file of the current directory within game/
define Chess_File = '00-chess-engine/'
define Image_File = 'images/'
define Audio_File = 'audio/'
define Bin_File = 'bin/' # stockfish binaries
define Chesspieces_File = Chess_File + Image_File + 'chesspieces/'

# file
define Chessboard_Image = Chess_File + Image_File + 'chessboard.png'
define Move_Audio = Chess_File + Audio_File + 'move.wav'
define Capture_Audio = Chess_File + Audio_File + 'capture.wav'
define Promotion_Audio = Chess_File + Audio_File + 'promotion.wav'
define Check_Audio = Chess_File + Audio_File + 'check.wav'
define Checkmate_Audio = Chess_File + Audio_File + 'checkmate.wav'
define Draw_Audio = Chess_File + Audio_File + 'draw.wav' # used for resign, stalemate, threefold, fifty-move
define Flip_Board_Audio = Chess_File + Audio_File + 'flip_board.wav'
...
define Loc_Legth = 90 # length of one side of a loc
...
define Promotion_White = 6 # Index_Max - 1
define Promotion_Black = 1 # Index_Min + 1
...
# Start Enum
# status code enum
define When_Checked = 1
define Threefold = 2
define Fifty_Moves = 3
...
init python:
    ...
    # make sure it can play in every OS
    stockfish_dir = os.path.join(renpy.config.gamedir, Chess_File, Bin_File)
    ...
    class ChessDisplayable(renpy.Displayable):
        ...
        def __init__(self, fen=Start_FEN, player_color=None, depth=10):
            ...
            # displayables
            self.selected_img = Solid(Selected_Color, xsize=Loc_Legth, ysize=Loc_Legth)
            self.legal_dst_img = Solid(Legal_DST_Color, xsize=Loc_Legth, ysize=Loc_Legth)
            self.highlight_img = Solid(Previous_Move_Color, xsize=Loc_Legth, ysize=Loc_Legth)
        ...
        def event(self, ev, x, y, st):
            ...
            # skip GUI interaction for AI's turn
        ...
        def has_promoting_piece(self, file_idx, rank_idx):
            ...
            if piece.color == chess.WHITE:
                return rank_idx == Promotion_White
            else:
                return rank_idx == Promotion_Black
